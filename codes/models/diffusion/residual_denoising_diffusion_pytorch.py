...
class Unet(nn.Module):
    ...
    def forward(self, x, x_input, x_ref, time, x_self_cond=None):
...
class UnetRes(nn.Module):
    ...
    def forward(self, x, x_input, x_ref, time, x_self_cond=None):
        if self.share_encoder == -1:
            ...
            return (self.unet0(x, x_input, x_ref, time, x_self_cond=x_self_cond),
                    self.unet1(x, x_input, x_ref, time, x_self_cond=x_self_cond))
...
class ResidualDiffusion(nn.Module):
    ...
    def model_predictions(self, x_input, x, x_ref, t, x_input_condition=0, x_self_cond=None, clip_denoised=True):
    ...
    @torch.no_grad()
    def ddim_sample(self, x_input, x_ref, shape, last=True):
        if self.input_condition:
            x_input_condition = x_input[1]
        else:
            x_input_condition = 0
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            self_cond = x_start if self.self_condition else None
            preds = self.model_predictions(
                x_input, img, x_ref, time_cond, x_input_condition, self_cond)
    ...
    @torch.no_grad()
    def sample(self, x_input=0, x_ref=0, batch_size=16, last=True):
        ...
        return sample_fn(x_input, x_ref, size, last=last)
        ...
        with torch.no_grad():
            ...
            all_images_list = show_x_input_sample + \
                list(self.ema.ema_model.sample(
                    x_input_sample, x_ref, batch_size=batches, last=last)) + \
                show_x_ref_sample
                ...
                with torch.no_grad():
                    ...
                    if sample:
                        ...
                        all_images_list = list(self.ema.ema_model.sample(
                            x_input_sample, x_ref, batch_size=batches))
