...
from codes.datasets.get_dataset import dataset
...
from codes.utils.Vgg19 import Vgg19
import time as time111
...
class GuidBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim=None, groups=8):
        ...
        self.block1 = Block(dim, dim_out, groups=groups)
        self.block2 = Block(dim_out*2, dim_out, groups=groups)
        self.res_conv = nn.Conv2d(
            dim, dim_out, 1) if dim != dim_out else nn.Identity()

    def forward(self, x, guid, time_emb=None):
...
class Unet(nn.Module):
    def __init__(
        ...
        self.init_conv = nn.Conv2d(6, init_dim, 7, padding=3)


        dims = [init_dim, *map(lambda m: dim * m, dim_mults), 512]  #  64 64 128 256 512 512
        in_out_down = list(zip(dims[1:], dims[2:]))    # (64,128)(128,256)(256,512)(512,512)
        in_out_up = list(zip(dims[:-3], dims[1:]))   # (64,64)(64,128)(128,256)(256,512)

        block_klass = partial(ResnetBlock, groups=resnet_block_groups)
        guid_block_klass = partial(GuidBlock, groups=resnet_block_groups)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out_down):
            ...
            # (64,128)(128,256)(256,512)(512,512)
        ...
        up_dim = 512
        self.up_block1 = block_klass(up_dim * 2, up_dim, time_emb_dim=time_dim)
        self.up_block2 = block_klass(up_dim * 2, up_dim, time_emb_dim=time_dim)
        self.up_attn = Residual(PreNorm(up_dim, LinearAttention(up_dim)))
        self.upsample = Upsample(up_dim, 256)

        for ind, (dim_in, dim_out) in enumerate(reversed(in_out_up)):
            is_last = ind == (len(in_out_up) - 1)
            # (64,64)(64,128)(128,256)(256,512)
            self.ups.append(nn.ModuleList([
                guid_block_klass(dim_out * 2, dim_out, time_emb_dim=time_dim),
                guid_block_klass(dim_out * 2, dim_out, time_emb_dim=time_dim),
                Residual(PreNorm(dim_out, LinearAttention(dim_out))),
                Upsample(dim_out, dim_in) if not is_last else nn.Conv2d(
                    dim_in, dim_in, 3, padding=1)
            ]))
    ...
    def forward(self, x, x_input, x_ref, time, x_self_cond=None, freeu=False):
        ...
        for block1, block2, attn, upsample in self.ups:
            x = torch.cat((x, h.pop()), dim=1)
            x = block1(x, x_ref[i], t)
            x = torch.cat((x, h.pop()), dim=1)
            x = block2(x, x_ref[i], t)
            x = attn(x)
            x = upsample(x)
            i = i+1
...
class UnetRes(nn.Module):
    def __init__(
        ...
        if self.share_encoder == -1:
            ...
            dims = [init_dim, *map(lambda m: dim * m, dim_mults), 512]  # 64 64 128 256 512 512
            in_out_down = list(zip(dims[1:], dims[2:]))  # (64,128)(128,256)(256,512)(512,512)
            in_out_up = list(zip(dims[:-2], dims[1:]))  # (64,64)(64,128)(128,256)(256,512)
            ...
            for ind, (dim_in, dim_out) in enumerate(reversed(in_out_up)):
                ...
                # (64,64)(64,128)(128,256)(256,512)
                self.ups.append(nn.ModuleList([
                    block_klass(dim_out * 2, dim_out, time_emb_dim=time_dim) if is_frist else block_klass(dim_out * 3,
                                                                                                          dim_out,
        ...
        if self.share_encoder == -1:
            ...
            for block1, block2, attn, upsample in self.ups:
                if i == 0:
                    ...
                    x = torch.cat((x, h.pop(), x_ref[i - 1]), dim=1)
            ...
            x = self.final_res_block(x, t)
            out_res, out_noise = self.final_conv(x).chunk(2, dim=1)

            return (out_res ,out_noise)
            ...
            for block1, block2, attn, upsample in self.ups_no_skip:
                if i == 0:
                    out_res = torch.cat((out_res, h.pop()), dim=1)
                else:
                    out_res = torch.cat((out_res, h.pop(), x_ref[i - 1]), dim=1)
            ...
            out_res = torch.cat((out_res, r), dim=1)
            ...
            out_noise = self.final_conv_2(x)
            return (out_res ,out_noise)

        elif self.share_encoder == 0:
            return (self.unet0(x, x_input, x_ref, time, x_self_cond=x_self_cond, freeu=freeu),
                    self.unet1(x, x_input, x_ref, time, x_self_cond=x_self_cond, freeu=freeu))
...
class ResidualDiffusion(nn.Module):
    ...
    def model_predictions(self, x_input, x, x_ref, t, x_input_condition=0, x_self_cond=None, clip_denoised=True, freeu=False):
    ...
    @torch.no_grad()
    def ddim_sample(self, x_input, x_ref, shape, last=True, freeu=False):
        if self.input_condition:
            x_input_condition = x_input[1]
        else:
            x_input_condition = 0
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            self_cond = x_start if self.self_condition else None
            preds = self.model_predictions(
                x_input, img, x_ref, time_cond, x_input_condition, self_cond, freeu=freeu)
        ...
        if self.condition:
            ...
            end_time = time111.perf_counter()
            total_time = end_time - start_time
            print(f"Sampling loop completed in {total_time:.4f} seconds")
    ...
    @torch.no_grad()
    def sample(self, x_input=0, x_ref=0, batch_size=16, last=True, freeu=False):
        ...
        return sample_fn(x_input, x_ref, size, last=last, freeu=freeu)
    ...
    def p_losses(self, imgs, ref_img, t, noise=None):
        ...
        if mix_loss:
            ...
            for i in range(len(model_out)):
                rn_loss = rn_loss + \
                    self.loss_fn(model_out[i], target[i])

            # pixel_loss
            pixel_loss = 5000 * self.loss_fn(x_u, u_gt)  # 1 - 0.5

            # per_loss
            sr_no_relu5_1 = self.vgg19((x_u + 1.) / 2.)
            with torch.no_grad():
                hr_no_relu5_1 = self.vgg19((u_gt.detach() + 1.) / 2.)
            per_loss = 3000 * F.mse_loss(sr_no_relu5_1, hr_no_relu5_1)  # 1 - 0.1

            loss = rn_loss + 0.1*pixel_loss + 0.1*per_loss

        return loss

    def forward(self, img, ref_img, *args, **kwargs):
        if isinstance(img, list):
        ...
        if self.input_condition and self.input_condition_mask:
            ...
            # ref_img = normalize_to_neg_one_to_one(ref_img)

        return self.p_losses(img, ref_img, t, *args, **kwargs)
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        if self.condition:
            if len(folder) == 3:
                ...
                self.sample_loader = cycle(
                    self.accelerator.prepare(DataLoader(self.sample_dataset, batch_size=num_samples, shuffle=False,
                                                        pin_memory=True, num_workers=4)))  # cpu_count()
                test_list = ds.gt
                ds = dataset(ref_folder, self.image_size, augment_flip=augment_flip,
                             convert_image_to=convert_image_to, condition=1, equalizeHist=equalizeHist,
                             crop_patch=crop_patch, generation=generation, train_list=test_list)
                self.rl_test_dataset = ds
                self.rl_test = cycle(self.accelerator.prepare(DataLoader(self.rl_test_dataset, batch_size=num_samples,
                                                                         shuffle=False, pin_memory=True,
                                                                         num_workers=4)))
                # train_gt+train_input
                ds = dataset(trian_folder, self.image_size,
                             augment_flip=augment_flip, convert_image_to=convert_image_to, condition=1,
                             equalizeHist=equalizeHist,
                ...
                ds = dataset(ref_folder, self.image_size, augment_flip=augment_flip,
                             convert_image_to=convert_image_to, condition=1, equalizeHist=equalizeHist,
                             crop_patch=crop_patch, generation=generation, train_list=train_list)
                self.rl = cycle(self.accelerator.prepare(DataLoader(ds, batch_size=train_batch_size,
                                                                    shuffle=False, pin_memory=True, num_workers=4)))
        ...
        self.opt_ref = Adam(refnet.parameters(), lr=train_lr, betas=adam_betas) if exists(self.refnet) else None
        # for logging results in a folder periodically

        if self.accelerator.is_main_process:
            self.ema = EMA(diffusion_model, beta=ema_decay, update_every=ema_update_every)
            self.ema_ref = EMA(refnet, beta=ema_decay, update_every=ema_update_every) if exists(self.refnet) else None
        ...
        self.refnet, self.opt_ref = self.accelerator.prepare(self.refnet, self.opt_ref) if exists(self.refnet) else None
    ...
    def save(self, milestone):
        ...
        data = {
            ...
            'refnet': self.accelerator.get_state_dict(self.refnet) if exists(self.refnet) else None,
            'opt_ref': self.opt_ref.state_dict() if exists(self.refnet) else None,
            'ema_ref': self.ema_ref.state_dict() if exists(self.refnet) else None
        }
    ...
    def load(self, milestone):
        ...
        if path.exists():
            ...
            self.opt_ref.load_state_dict(data['opt_ref'])
            self.ema_ref.load_state_dict(data['ema_ref'])
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    if self.condition:
                        ...
                        data_ref = next(self.rl)
                        data_ref = [item.to(self.device) for item in data_ref]
                    else:
                        data = next(self.dl)
                    ...
                    with self.accelerator.autocast():
                        T_list = self.refnet(lrsr=data[1], ref=data_ref[0], refsr=data_ref[1])

                        loss = self.model(data, T_list)
                        loss = loss / self.gradient_accumulate_every
                        total_loss = total_loss + loss.item()

                    self.accelerator.backward(loss)

                accelerator.clip_grad_norm_(self.refnet.parameters(), 1.0)
                accelerator.clip_grad_norm_(self.model.parameters(), 1.0)

                accelerator.wait_for_everyone()

                self.opt_ref.step()
                self.opt_ref.zero_grad()
                self.opt.step()
                self.opt.zero_grad()
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        avg_loss = cycle_loss / cycle_steps if cycle_steps > 0 else 0
    ...
    def sample(self, milestone, last=True, FID=False):
        ...
        with torch.no_grad():
            ...
            if self.condition_type == 0:
                ...
                x_ref_sample = next(self.rl_test)
                x_ref_sample = [item.to(self.device)
                                  for item in x_ref_sample]
                show_x_ref_sample = x_ref_sample
                x_ref = self.refnet(show_x_input_sample[1], show_x_ref_sample[0], show_x_ref_sample[1])
            ...
            all_images_list = show_x_input_sample + \
                list(self.ema.ema_model.sample(
                    x_input_sample, x_ref, batch_size=batches, last=last, freeu=True)) + \
                show_x_ref_sample
    ...
    def test(self, sample=False, last=True, FID=False):
        ...
        if self.condition:
            ...
            ref_loader = DataLoader(
                dataset=self.rl_test_dataset,
                batch_size=1)

            i = 0
            for items, ref_items in zip(loader, ref_loader):
                if self.condition:
                ...
                with torch.no_grad():
                    ...
                    if self.condition_type == 0:
                        ...
                        x_ref = self.refnet(show_x_input_sample[1], x_ref_sample[0], x_ref_sample[1])
