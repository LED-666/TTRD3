...
class Dataset(Dataset):
    def __init__(
        ...
        self.sample = sample
        if exists(train_list):
            if condition == 1:
                # condition
                self.gt, self.input = self.filter_ref_images(train_list, folder[0], folder[1])
            elif condition == 0:
                # generation
    ...
    def __getitem__(self, index):
        if self.condition == 1: # 对输入的高分辨率和低分辨率图像进行预处理和增强，包括填充、裁剪、直方图均衡化和翻转等操作，最终返回两个图像的张量
            # condition
    ...
    def filter_ref_images(self, train_list, ref_folder, ref_sr_folder):
        # 提取图片名称（不包括路径）
        train_image_names = [os.path.basename(image) for image in train_list]

        # 读取ref文件夹中的所有图片名称
        ref_images = self.load_flist(ref_folder)
        ref_image_names = [os.path.basename(image) for image in ref_images]
        ...
        for name in train_image_names:
            ...
            # 在ref中找到以相同前缀开头的图片
            matched_names = [img for img in ref_image_names if img.split('_')[0] == prefix]
            # matched_names = [img for img in ref_image_names if img.startswith(prefix)]
            if matched_names:
                # 从匹配的图片中随机选择一张
                selected_image = random.choice(matched_names)
                matched_ref_images.append(os.path.join(ref_folder, selected_image))

        matched_ref_sr_images = []
        ref_sr_images_names = [os.path.basename(image) for image in matched_ref_images]
        for name in ref_sr_images_names:
            matched_ref_sr_images.append(os.path.join(ref_sr_folder, name))

        return matched_ref_images, matched_ref_sr_images
